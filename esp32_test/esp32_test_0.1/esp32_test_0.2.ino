...
const char* mqtt_id = "ESP32TEST";
const char* mqtt_user = "servbd";
const char* mqtt_password = "Un1f3sp1";
const char* controlTopic = "sensores/control";
const char* statusSensoresTopic = "sensoresTest/status";
const char* statusESPTopic = "espTest/status";
const char* statusNetTopic = "conexoesTest/status";
const char* dataDhtTopic = "sensoresTest/dht";
const char* dataMlxTopic = "sensoresTest/mlx";
...
void reconnectToBrokerMqtt() {
  while (!client.connected()) {
    ...
    if (client.connect(mqtt_id, mqtt_user, mqtt_password)) {
      ...
      client.publish(statusNetTopic, "[MQTT-TEST]: CONECTADO");
    } else {
  ...
  if (message.indexOf("START") >= 0) {
    ...
    if (!bufferDHT.empty() || !bufferMLX.empty()) {
      ...
      if (taskHandleDHT != NULL) {
        ...
        taskHandleDHT = NULL;
        Serial.println("[ESP32-TEST-TASK]: TASK_DHT_ENCERRADA");
        client.publish(statusESPTopic, "[ESP32-TEST-TASK]: TASK_DHT_ENCERRADA");
      }

      if (taskHandleMLX != NULL) {
        vTaskDelete(taskHandleMLX);
        taskHandleMLX = NULL;
        Serial.println("[ESP32-TEST-TASK]: TASK_MLX_ENCERRADA");
        client.publish(statusESPTopic, "[ESP32-TEST-TASK]: TASK_MLX_ENCERRADA");
      }
    }
  }
}
  ...
  for (;;) {
    if (collecting && dhtEnabled) {
      ...
      if (!isnan(temperatura) && !isnan(umidade)) {
        dado.v1 = temperatura;
        dado.v2 = umidade;
        Serial.printf("[DHT22-TEST]: Temperatura: %.1f°C Umidade: %.1f%%\n", temperatura, umidade);
      } else {
        dado.v1 = 0.0;
        dado.v2 = 0.0;
        dht.begin();
        Serial.println("[DHT22-TEST]: ERROR_LEITURA");
        client.publish(statusSensoresTopic, "[DHT22-TEST]: ERROR_LEITURA");
      }
  ...
  for (;;) {
    if (collecting && mlxEnabled) {
      ...
      if (!isnan(tempA) && !isnan(tempIR)) {
        dado.v1 = tempA;
        dado.v2 = tempIR;
        Serial.printf("[MLX90614-TEST]: Temp-Amb: %.1f°C Temp-IR: %.1f°C\n", tempA, tempIR);
      } else {
        dado.v1 = 0.0;
        dado.v2 = 0.0;
        mlx.begin();
        Serial.println("[MLX90614-TEST]: ERROR_LEITURA");
        client.publish(statusSensoresTopic, "[MLX90614-TEST]: ERROR_LEITURA");
      }
...
void publicarBufferMQTT() {
  ...
  if (bufferDHT.size() > LIMITE_BUFFER || bufferMLX.size() > LIMITE_BUFFER) {
    Serial.printf("[MQTT-TEST-BUFFER]: ERROR_BUFFER_LIMITE-MAX");
    client.publish(statusESPTopic, "[MQTT-TEST-BUFFER]: ERROR_BUFFER_LIMITE-MAX");
  }

  if (bufferDHT.empty() && bufferMLX.empty()) {
    Serial.println("[MQTT-TEST-BUFFER]: ERROR_BUFFER");
    client.publish(statusESPTopic, "[MQTT-TEST-BUFFER]: ERROR_BUFFER");
    return;
  }
  ...
  if (!bufferDHT.empty()) {
    for (int i = 0; i < bufferDHT.size(); i += LINHAS_POR_PUBLICACAO) {
      ...
      if (client.publish(dataDhtTopic, csv)) {
        Serial.println("[MQTT-TEST]: DHT22-TEST_PUBLICADO");
      } else {
        Serial.println("[MQTT-TEST]: ERROR_PUBLICAR_DHT22-TEST");
        client.publish(statusESPTopic, "[MQTT-TEST]: ERROR_PUBLICAR_DHT22-TEST");
      }
    }
    bufferDHT.clear();
  ...
  if (!bufferMLX.empty()) {
    for (int i = 0; i < bufferMLX.size(); i += LINHAS_POR_PUBLICACAO) {
      ...
      if (client.publish(dataMlxTopic, csv)) {
        Serial.println("[MQTT-TEST]: MLX90614-TEST_PUBLICADO");
      } else {
        Serial.println("[MQTT-TEST]: ERROR_PUBLICAR_MLX90614-TESTE");
        client.publish(statusSensoresTopic, "[MQTT-TEST]: ERROR_PUBLICAR_MLX90614-TEST");
      }
    }
    bufferMLX.clear();
...
void loop() {
  ...
  if (millis() - lastHeartbeat >= 10000) {
    ...
    char payload[80];
    snprintf(payload, sizeof(payload), "[MQTT-TEST]: OK | %s | %lus", hora, millis() / 1000);
