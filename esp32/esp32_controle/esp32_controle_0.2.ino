...
const char* statusSensoresTopic = "sensoresControle/status";
const char* statusESPTopic = "espControle/status";
const char* statusNetTopic = "conexoesControle/status";
const char* dataDhtTopic = "sensoresControle/dht";
const char* dataMlxTopic = "sensoresControle/mlx";
  ...
  for (;;) {
    if (collecting && dhtEnabled) {
      ...
      if (!isnan(temperatura) && !isnan(umidade)) {
        ...
        client.publish(statusSensoresTopic, "[DHT22-CONTROLE]: ERROR_LEITURA");
      }
  ...
  for (;;) {
    if (collecting && mlxEnabled) {
      ...
      if (!isnan(tempA) && !isnan(tempIR)) {
        ...
        client.publish(statusSensoresTopic, "[MLX90614-CONTROLE]: ERROR_LEITURA");
      }
...
void publicarBufferMQTT() {
  ...
  if (bufferDHT.size() > LIMITE_BUFFER || bufferDHT.size() > LIMITE_BUFFER) {
    Serial.printf("[MQTT-CONTROLE-BUFFER]: ERROR_BUFFER_LIMITE-MAX");
    client.publish(statusESPTopic, "[MQTT-CONTROLE-BUFFER]: ERROR_BUFFER_LIMITE-MAX");
  }

  if (bufferDHT.empty() && bufferMLX.empty()) {
    Serial.println("[MQTT-CONTROLE-BUFFER] ERROR_BUFFER");
    client.publish(statusESPTopic, "[MQTT-CONTROLE-BUFFER] ERROR_BUFFER");
    return;
  }
  ...
  if (!bufferDHT.empty()) {
    for (int i = 0; i < bufferDHT.size(); i += LINHAS_POR_PUBLICACAO) {
      ...
      if (client.publish(dataDhtTopic, csv)) {
        ...
        client.publish(statusESPTopic, "[MQTT-CONTROLE]: ERROR_PUBLICAR_DHT22-CONTROLE");
      }
    }
    bufferDHT.clear();
  ...
  if (!bufferMLX.empty()) {
    for (int i = 0; i < bufferMLX.size(); i += LINHAS_POR_PUBLICACAO) {
      ...
      if (client.publish(dataMlxTopic, csv)) {
        ...
        client.publish(statusSensoresTopic, "[MQTT-CONTROLE] ERROR_PUBLICAR_MLX90614-CONTROLE");
      }
    }
    bufferMLX.clear();
...
void loop() {
  ...
  if (millis() - lastHeartbeat >= 10000) {
    ...
    char payload[80];
    snprintf(payload, sizeof(payload), "[MQTT-CONTROLE]: OK | %s | %lus", hora, millis() / 1000);
